...
export function isBookingCalendarSynced(status: string | null | undefined) {
  return status === "synced";
}

export function BookingCalendarSyncControl({
  bookingId,
  syncStatus,
}: {
  bookingId: string;
  ...
  const synced = optimisticSynced || isBookingCalendarSynced(syncStatus);
  ...
  async function performSync() {
    ...
    if (syncStatus === "needs_sync" || syncStatus === "error") {
      await google.updateBookingEvent(bookingId);
      return;
    }
    await google.createBookingEvent(bookingId);
  }
  ...
  return (
    <ListRowActions className="inline-flex shrink-0">
      ...
      >
        ...
        <span className="text-[11px] font-semibold leading-none">Sync now</span>
      </button>
    </ListRowActions>
  );
}
